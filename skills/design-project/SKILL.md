...
2. **Ensures the preview server is running** by calling `mcp__claude-design__design_start`. This is a no-op if the server is already up.
...
4. **Retrieves the preview URL** using `mcp__claude-design__design_url` once the subagent completes.
